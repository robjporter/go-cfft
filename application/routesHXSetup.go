  ...
  if err == nil {
    if a.HX.GetResponseOK(res) {
      if a.HX.GetResponseCode(res) == 201 {
        ...
        a.Logger.WithFields(logrus.Fields{"Error":err,"Code":a.HX.GetResponseCode(res)}).Debug("We received an error and cannot continue.")
      }
    } else {
      a.LastError = errors.New("We recevied a response which was invalid.")
      a.Logger.WithFields(logrus.Fields{"Error":err,"OK":a.HX.GetResponseOK(res)}).Debug("We received an error and cannot continue.")
    }
  } else {
