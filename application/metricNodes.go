...
func (a *Application) metricGetFurtherNodeInfo(nodes *MetricNodes) *MetricNodes {
	...
	if err != nil {
		a.Logger.WithFields(logrus.Fields{"Task Number": a.Stats.GetCounter("tasks")}).Debug("We were unable to collect the Appliance information from HX Connect API.")
		a.LastError = err
		return &MetricNodes{}
	}
	...
	if a.HX.GetResponseOK(res) {
		if a.HX.GetResponseCode(res) == 200 {
			a.Logger.WithFields(logrus.Fields{"Task Number": a.Stats.GetCounter("tasks")}).Debug("Querying HX Connect for Appliance information.")
			...
			if a.HX.GetResponseItemInt(res, "#") == nodes.NodeCount {
				...
				nodes2.Nodes = &nodess
				a.Logger.WithFields(logrus.Fields{"Task Number": a.Stats.GetCounter("tasks")}).Debug("Querying HX Connect for Appliance information complete.")
				return &nodes2
			}
			a.Logger.WithFields(logrus.Fields{"Task Number": a.Stats.GetCounter("tasks"), "Number received": a.HX.GetResponseItemInt(res, "#"), "Number expected": nodes.NodeCount}).Warning("The number of nodes reported is different to the number expected.")
		}
		a.Logger.WithFields(logrus.Fields{"Task Number": a.Stats.GetCounter("tasks"), "ResponseCode": a.HX.GetResponseCode(res)}).Warning("An unexpected response code was received for Appliance information.")
	} else {
		a.Logger.WithFields(logrus.Fields{"Task Number": a.Stats.GetCounter("tasks"), "ResponseOK": false}).Warning("We received a failed attempt at connecting to the Appliance endpoint.")
	}
	return &MetricNodes{}
}
